<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-ys-l text-dc">Редактировать закон</h2></x-slot>
    <div class="py-8 max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-surface rounded-md shadow-dc-card p-6">
            <form method="POST" action="{{ route('laws.update', $law) }}" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                        class="w-full rounded-2xs border border-dc-gray-20 bg-surface text-dc text-ys-s px-3 py-2 focus:outline-none focus:border-dc-blue-100">{{ old('description', $law->description) }}</textarea>
                </div>
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('laws.show', $law) }}" class="px-5 py-2 text-dc-secondary hover:text-dc text-ys-s dc-transition">Отмена</a>
                </div>
            </form>
        </div>
    </div>
