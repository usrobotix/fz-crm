<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('templates.show', $template) }}" class="text-dc-secondary hover:text-dc text-ys-s">← {{ $template->title }}</a>
            <h2 class="font-semibold text-ys-l text-dc">Редактировать шаблон</h2>
        </div>
    </x-slot>
        ...
        <form method="POST" action="{{ route('templates.update', $template) }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-surface rounded-md shadow-dc-card p-5 space-y-4">
                    ...
                    <div>
                        <label class="block text-ys-s font-medium text-dc mb-1">Закон <span class="text-dc-red-100">*</span></label>
                        <select name="law_id" required
                            ...
                            @foreach($laws as $law)
                            <option value="{{ $law->id }}" {{ old('law_id', $template->law_id) == $law->id ? 'selected' : '' }}>{{ $law->code }} — {{ $law->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="doc_type" value="{{ old('doc_type', $template->doc_type) }}"
                            class="w-full rounded-2xs border border-dc-gray-20 bg-surface text-dc text-ys-s px-3 py-2 focus:outline-none focus:border-dc-blue-100">
                    </div>
                    <div>
                        <label class="block text-ys-s font-medium text-dc mb-1">Источник</label>
                        <input type="text" name="source" value="{{ old('source', $template->source) }}"
                            class="w-full rounded-2xs border border-dc-gray-20 bg-surface text-dc text-ys-s px-3 py-2 focus:outline-none focus:border-dc-blue-100">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="repo_path" value="{{ old('repo_path', $template->repo_path) }}"
                    ...
                    <div class="pt-2 flex gap-3">
                        ...
                        <a href="{{ route('templates.show', $template) }}" class="px-5 py-2 text-dc-secondary hover:text-dc text-ys-s dc-transition">Отмена</a>
                    </div>
                </div>
